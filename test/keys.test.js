...
test('makeKey/parseKey round-trip with uploader name', () => {
  const key = makeKey('jpg', 'Auntie Sue!', new Date('2026-09-12T18:30:00.123Z'));
  assert.match(key, /^photos\/20260912T183000123_[a-f0-9]{8}_Auntie-Sue\.jpg$/);
...
test('parseKey rejects foreign keys', () => {
  assert.equal(parseKey('photos/../../etc/passwd'), null);
  assert.equal(parseKey('thumbs/20260912T183000123_deadbeef.webp'), null);
  assert.equal(parseKey('photos/table-05/20260912T183000123_deadbeef.jpg'), null);
  assert.equal(parseKey(''), null);
});

test('keys sort newest-first by timestamp segment', () => {
  const older = makeKey('jpg', '', new Date('2026-09-12T18:00:00Z'));
  const newer = makeKey('jpg', '', new Date('2026-09-12T21:15:00Z'));
  assert.ok(parseKey(newer).ts > parseKey(older).ts);
});

test('thumbKeyFor maps original to webp thumb', () => {
  assert.equal(
    thumbKeyFor('photos/20260912T183000123_deadbeef_Dave.heic'),
    'thumbs/20260912T183000123_deadbeef_Dave.webp',
  );
});
