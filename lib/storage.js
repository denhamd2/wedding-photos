...
//   r2    — Cloudflare R2 via the S3 API (production; presigned PUTs go straight to the bucket)
//   local — plain filesystem (development/tests; "presigned" URLs point back at the app)
// Interface: putObject, getObject, headObject, deleteObject, listAll, presignPut
...
class LocalDriver {
  ...
  // The dev "presigned URL" is just a PUT endpoint on our own server.
  async presignPut(key, contentType) {
    return { url: `/api/dev-upload?key=${encodeURIComponent(key)}`, headers: { 'Content-Type': contentType } };
  }
}
...
function wrap(impl) {
  return {
    ...
    presignPut: impl.presignPut.bind(impl),
  };
}
