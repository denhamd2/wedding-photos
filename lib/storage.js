...
class LocalDriver {
  ...
  async getObject(key, range) {
    ...
    if (range) {
      ...
      return {
        body: fs.createReadStream(p, { start, end }),
        contentType,
    ...
    return { body: fs.createReadStream(p), contentType, contentLength: stat.size, totalLength: stat.size };
  }
...
class R2Driver {
  ...
  async getObject(key, range) {
    ...
    const result = { body: res.Body, contentType: res.ContentType, contentLength: res.ContentLength };
    if (res.ContentRange) {
      // "bytes start-end/total"
      const m = /bytes (\d+)-(\d+)\/(\d+)/.exec(res.ContentRange);
      if (m) {
        result.range = { start: +m[1], end: +m[2] };
        result.totalLength = +m[3];
      }
    } else {
      result.totalLength = res.ContentLength;
    }
    return result;
  }
